...
def process_task(task_data: List[Dict], task_id: int) -> None:
    ...
    try:
        # 根據任務ID設置不同的延遲時間，錯開模型加載時間
        start_delay = (task_id - 1) * 10  # 每個任務錯開10秒
        if start_delay > 0:
            print(f"\n任務 {task_id} 等待 {start_delay} 秒後開始...")
        ...
        # 創建臨時輸出文件
        out_path = Path(f"data/proc/labeled_temp_{task_id}.jsonl")
...
def combine_temp_files():
    ...
    success = True
    try:
        with Path("data/proc/labeled.jsonl").open("w") as outfile:
            for i in range(1, 5):  # 1 到 4
                temp_file = Path(f"data/proc/labeled_temp_{i}.jsonl")
                if temp_file.exists():
                    try:
...
def main():
    ...
    # 從新的 JSON 文件加載訓練數據
    with open("data/cnn_dailymail_train.json", 'r', encoding='utf-8') as f:
        dataset = json.load(f)
