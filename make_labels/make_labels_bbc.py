...
def process_task(task_data: List[Dict], task_id: int) -> None:
    ...
    try:
        ...
        # 創建臨時輸出文件
        out_path = Path(f"data/proc/labeled_bbc_temp_{task_id}.jsonl")
...
def combine_temp_files():
    ...
    success = True
    try:
        with Path("data/proc/labeled_bbc.jsonl").open("w", encoding="utf-8") as outfile:
            for i in range(1, 5):  # 1 到 4
                temp_file = Path(f"data/proc/labeled_bbc_temp_{i}.jsonl")
                if temp_file.exists():
                    try:
...
def main():
    ...
    # 加載數據集
    print("正在加載數據集...")
    ds = load_dataset("json", data_files="data/raw/bbc_5pct.jsonl")["train"]
